...
public:
    ...
    DatabaseWorkerConfig loadWorkerConfig(QString* errorMessage = nullptr) const;
    QVariantMap workerConfigDraft(QString* errorMessage = nullptr) const;
    DatabaseOperationResult saveWorkerConfig(const QVariantMap& draft, bool clearApiKey = false) const;
    DatabaseOperationResult copyMainModelConfig(const QJsonObject& mainSettings) const;
    ...
    QList<DatabaseTurnView> recentTurns(const QString& cardUid, int limit = 50) const;
